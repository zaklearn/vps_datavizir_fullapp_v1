"""
Module for providing educational interpretations of correlation patterns.
"""
import streamlit as st
from config import egra_columns, egma_columns

def provide_educational_interpretation(df_strong, t):
    """
    Provides educational interpretations of correlation patterns.
    ...
    # Start with cross-domain correlations (EGRA-EGMA) as they're most interesting
    if egra_egma:
        display_cross_domain_interpretations(egra_egma, t)
    
    # EGRA-EGRA correlations
    if egra_egra:
        display_reading_interpretations(egra_egra, t)
    
    # EGMA-EGMA correlations
    if egma_egma:
        display_math_interpretations(egma_egma, t)
    
    # Overall instructional recommendations
    display_instructional_recommendations(egra_egma, egra_egra, egma_egma, t)

def display_cross_domain_interpretations(egra_egma, t):
    """
    Display interpretations for cross-domain correlations.
    
    Args:
        egra_egma (list): List of EGRA-EGMA correlation tuples
        t (dict): Translation dictionary for UI elements
    """
    st.markdown(f"#### {t.get('cross_domain_title', 'Reading-Math Connections')}")
    st.markdown(t.get("cross_domain_intro", "These correlations indicate connections between reading and math skills:"))
    ...
    for task1, task2, correlation in egra_egma:
        ...
        st.markdown(f"**{egra_name} & {egma_name}** ({corr_str})")
...
def display_reading_interpretations(egra_egra, t):
    """
    Display interpretations for reading skill correlations.
    
    Args:
        egra_egra (list): List of EGRA-EGRA correlation tuples
        t (dict): Translation dictionary for UI elements
    """
    st.markdown(f"#### {t.get('reading_correlations_title', 'Within Reading Skills')}")
    st.markdown(t.get("reading_correlations_intro", "These correlations show relationships between different reading skills:"))
    ...
    for task1, task2, correlation in egra_egra:
        ...
        st.markdown(f"**{task1_name} & {task2_name}** ({corr_str})")
        
        # Provide reading-specific interpretation
        interpretation = get_reading_interpretation(task1, task2, correlation, t)
        st.markdown(f"- {interpretation}")

def display_math_interpretations(egma_egma, t):
    """
    Display interpretations for math skill correlations.
    
    Args:
        egma_egma (list): List of EGMA-EGMA correlation tuples
        t (dict): Translation dictionary for UI elements
    """
    st.markdown(f"#### {t.get('math_correlations_title', 'Within Math Skills')}")
    st.markdown(t.get("math_correlations_intro", "These correlations indicate relationships between different math skills:"))
...
def display_instructional_recommendations(egra_egma, egra_egra, egma_egma, t):
    """
    Display instructional recommendations based on correlation patterns.
    ...
    Args:
        ...
        t (dict): Translation dictionary for UI elements
    """
    st.markdown(f"#### {t.get('instructional_recommendations', 'Instructional Recommendations')}")
    
    if egra_egma:
        st.markdown(f"**{t.get('integrated_instruction', 'Integrated Instruction')}**")
        st.markdown(t.get("integrated_instruction_text", "- Consider integrated lessons that combine reading and math skills that show strong correlations"))
        st.markdown(t.get("integrated_instruction_example", "- For example, use story problems that incorporate both mathematical thinking and reading comprehension"))
    
    if egra_egra:
        st.markdown(f"**{t.get('reading_instruction', 'Reading Instruction')}**")
        if any(correlation < 0 for _, _, correlation in egra_egra):
            st.markdown(t.get("reading_negative_text", "- Pay attention to negative correlations that may indicate competing skills or resource allocation"))
        st.markdown(t.get("reading_positive_text", "- Build on strong positive correlations by teaching related skills together"))
    
    if egma_egma:
        st.markdown(f"**{t.get('math_instruction', 'Math Instruction')}**")
        if any(correlation < 0 for _, _, correlation in egma_egma):
            st.markdown(t.get("math_negative_text", "- Address negative correlations by ensuring students understand connections between skills"))
        st.markdown(t.get("math_positive_text", "- Leverage strong positive correlations to build mathematical understanding"))
...
def get_instructional_implication(egra_task, egma_task, correlation, t):
    ...
    # Default implications based on skill categories
    # Decoding tasks
