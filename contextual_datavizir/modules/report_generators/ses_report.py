"""
SES and home support report generator for EduInsight.

This module generates comprehensive reports analyzing how socioeconomic status (SES)
and home support affect student performance.
"""
...
def generate_ses_report(data, t, doc=None):
    """
    Generate a SES and home support analysis report.
    ...
    Returns:
        Document: Word document with SES and home support report
    """
    # Extract data
    ...
    # Add title
    doc.add_heading(t.get("title_ses_home_support", "SES & Home Support Analysis"), level=0)
    
    # Add introduction
    doc.add_paragraph(t.get("ses_home_support_intro", 
        ...
        try:
            ...
            if not heatmap_pivot.empty:
                ...
                if 'Low' in heatmap_pivot.index and 'High' in heatmap_pivot.columns:
                    low_ses_high_support = heatmap_pivot.loc['Low', 'High']
                    
                    if 'High' in heatmap_pivot.index and 'Low' in heatmap_pivot.columns:
                        high_ses_low_support = heatmap_pivot.loc['High', 'Low']
            ...
            doc.add_paragraph(t.get("heatmap_error", "Could not create interaction heatmap due to data limitations."))
...
def _add_correlation_analysis(doc, df_analysis, selected_columns, t):
    ...
    if len(numeric_vars) >= 2:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            fig.update_layout(
                title=t.get("correlation_matrix_title", "Correlation Matrix of SES, Home Support, and Performance"),
                height=600,
...
def _add_recommendations(doc, df_analysis, selected_columns, t):
    """
    Add recommendations based on SES and home support analysis.
    ...
    if all(col in df_analysis.columns for col in ["ses_group", "home_support_group", "total_score"]):
        try:
            ...
            low_ses_high_support_key = ('Low', 'High')
            high_ses_low_support_key = ('High', 'Low')
            
            if interaction_data.index.isin([low_ses_high_support_key]).any() and interaction_data.index.isin([high_ses_low_support_key]).any():
                low_ses_high_support = interaction_data.loc[low_ses_high_support_key]
                high_ses_low_support = interaction_data.loc[high_ses_low_support_key]
            ...
            if not interaction_data.empty:
                min_idx = interaction_data.idxmin()
                if isinstance(min_idx, tuple) and len(min_idx) == 2:
                    low_ses_group, low_support_group = min_idx
